...
def clear(): #길진 않지만 자주 쓰는 코드는 짧은 단어의 함수로 미리 만들어 놓음
    os.system('cls' if os.name == 'nt' else 'clear') #터미널 공백 만들기
...
RESET = "\033[0m"
NORM = "\033[22m"
BOLD = "\033[1m" # 29~36 : 연출에 대한 자세한 지시사항
...
def tte_slide_print(text): #왼쪽에서부터 하나씩 출력되는 연출을 작동하는 함수
    ...
    try:
        ...
        slow_print(text) #작동하지 않는다면 slow_print함수로 보내기

def tte_fireworks_reveal(text): #폭죽 연출을 만드는 함수
    ...
    slow_print(text, delay=0.03) #slow_print 함수 사용
...
def choose_reward(score, result): #보상을 선택하는 함수 / score와 result를 받음
    dist = reward_distribution(score, result) #reward_distribution 함수에서 score와 result를 주고 값을 받아 저장함
    rewards = list(dist.keys()) #딕셔너리 dist의 key들을 reward(보상) 리스트로 저장함
    probs = list(dist.values()) #딕셔너리 dist의 value들을 probs(확률) 리스트를 저장함
    return random.choices(rewards, weights=probs, k=1)[0] #임의로 선택한 값을 함수 밖으로 내보냄.

def color_score(n): #score에 따라 달라지는 컬러값을 조정하는 함수 / n을 받음
    try: 
...
def animate_prize_draw(dist): #보상을 뽑는 연출을 만드는 함수 / dist값을 받음
    ...
    if n <= 1: #만약에 n이 1보다 작거나 같다면
        ...
        if TTE_AVAILABLE: #만약에 TTE_AVAILABLE이라면
            tte_fireworks_reveal(reveal) #reveal에 폭주효과를 넣음
        else: #아니라면
            slow_print(reveal, delay=0.1) #slow_print함수를 작동함
    ...
    if TTE_AVAILABLE: #만약 True라면
        tte_fireworks_reveal(reveal) #reveal에 폭죽 연출을 추가함
    else: #아니라면
        slow_print(reveal, delay=0.05) #함수 slow_print에 reveal과 delay를 넣어 실행
...
def animate_final_prize(drawn_items): #drawn_items를 넣고 최종 당첨 항목을 연출과 함께 출력하는 함수
    ...
    final_label = animate_prize_draw(dist) #animate_prize_draw에 dist를 넣고 나온 값을 저장
...
def final_draw_and_return(drawn_items):
    clear()
    if not drawn_items:
        return None
    print()
    winner = animate_final_prize(drawn_items)
    return winner
...
def show_ranking():
    while True:
        clear()
        print_final_ranking(GLOBAL_SCOREBOARD)
        print()
        ...
        if resp == "r":
            ...
            if confirm == "y":
                ...
                save_scoreboard("ranking.json", GLOBAL_SCOREBOARD)
                clear()
                print("랭킹보드를 초기화했습니다.")
                time.sleep(1.0)
            continue
...
def merge_scoreboard(scoreboard, new_entry):
...
def save_scoreboard(path, scoreboard):
    try:
...
def compute_draws_sum(draws):
    return sum(grade_to_num(normalize_grade(d)) for d in draws)

def print_final_ranking(scoreboard):
    grade_map = {"1 등급":1, "2 등급":2, "3 등급":3, "4 등급":4, "참가상":5}
    def sort_key(x):
        final = normalize_grade(x.get("final"))
        draws = x.get("draws", [])
        return (grade_map.get(final, 4), sum(grade_map.get(normalize_grade(d), 4) for d in draws), x.get("id",""))
    ...
    for idx, entry in enumerate(scoreboard_sorted, start=1):
        sid = entry.get("id","")
        final = normalize_grade(entry.get("final"))
        draws = [normalize_grade(d) for d in entry.get("draws",[])]
...
"""
    ...
    while True:
        ...
        display_counts_table(deck.counts())
        print()
        ...
        if drawn_items:
            print("\n--- 최종 추첨을 진행합니다 ---")
            final = final_draw_and_return(drawn_items)
            if final is None:
                final = drawn_items[-1] if drawn_items else "참가상"
            print(f"학번 {sid} 최종 상품: {final}")
            print(f"획득 상품 목록: {drawn_items}")
            entry = {"id": sid, "final": normalize_grade(final), "draws": [normalize_grade(d) for d in drawn_items]}
            merge_scoreboard(scoreboard, entry)
            GLOBAL_SCOREBOARD = scoreboard
            save_scoreboard(scoreboard_file, scoreboard)
        else:
            print(f"학번 {sid} 뽑기 결과가 없어 기록에 추가하지 않습니다.")
    print_final_ranking(scoreboard)
    print()
