...
def black_jack(n, m):
    ...
    # return card_num
    for i in range(n):
        card_plus_code = (card_num[i+a]) + (card_num[i+b]) + (card_num[i+c])
        if i+a > n-1:
            i+a == (i+a) -5
        if i+b > n-1:
            i+b == (i+b) -5
        ...
        # card_plus_code = (card_num[i+a]) + (card_num[i+b]) + (card_num[i+c])
        if int(card_plus_code) <= m:
            lst.append(int(card_plus_code))
    return lst


input_data = input().split(" ")
print(black_jack(int(input_data[0]), int(input_data[1])))
